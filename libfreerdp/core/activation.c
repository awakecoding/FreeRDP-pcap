...
void rdp_write_client_persistent_key_list_pdu(wStream* s, RDP_BITMAP_PERSISTENT_INFO* info)
{
	int index;
	UINT32 key1;
	UINT32 key2;
...
BOOL rdp_send_client_persistent_key_list_pdu(rdpRdp* rdp)
{
	...
	info.numEntriesCache0 = (keyCount < info.totalEntriesCache0) ? keyCount : info.totalEntriesCache0;
	keyCount -= info.numEntriesCache0;
	info.numEntriesCache1 = (keyCount < info.totalEntriesCache1) ? keyCount : info.totalEntriesCache1;
	keyCount -= info.numEntriesCache1;
	info.numEntriesCache2 = (keyCount < info.totalEntriesCache2) ? keyCount : info.totalEntriesCache2;
	keyCount -= info.numEntriesCache2;
	info.numEntriesCache3 = (keyCount < info.totalEntriesCache3) ? keyCount : info.totalEntriesCache3;
	keyCount -= info.numEntriesCache3;
	info.numEntriesCache4 = (keyCount < info.totalEntriesCache4) ? keyCount : info.totalEntriesCache4;
	...
	s = rdp_data_pdu_init(rdp);
	rdp_write_client_persistent_key_list_pdu(s, &info);

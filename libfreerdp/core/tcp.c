...
static BOOL transport_bio_pcap_read_ethernet_header(FILE* fp, PCAP_ETHERNET_HEADER* ethernet)
{
	wStream* s;
	BYTE buffer[14];
...
static BOOL transport_bio_pcap_read_ipv4_header(FILE* fp, PCAP_IPV4_HEADER* ipv4)
{
	wStream* s;
	BYTE bytes8;
	UINT16 bytes16;
...
static BOOL transport_bio_pcap_read_tcp_header(FILE* fp, PCAP_TCP_HEADER* tcp)
{
	wStream* s;
	BYTE bytes8;
	BYTE buffer[20];
...
static BOOL transport_bio_pcap_next(BIO* bio)
{
	...
	if (!ptr->plength)
	{
		...
		if (!transport_bio_pcap_read_ethernet_header(ptr->pcap->fp, &ptr->ethernet))
			return FALSE;
